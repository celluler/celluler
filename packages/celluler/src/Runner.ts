...
/* eslint-disable class-methods-use-this,global-require,import/no-dynamic-require,max-len */
...
import { ServiceBroker, ServiceSchema, Utils } from 'moleculer'
import os from 'os'
import path from 'path'

const utils: Utils = require('moleculer').Utils
...
export default class MoleculerRunner {
  ...
  private whiteListExt: string[] = ['.json', '.js', '.ts']
  ...
  */
  ...
  loadConfigFile() {
    ...
    if (filePath) {
      ...
      if (!this.whiteListExt.includes(ext)) {
      ...
      return Promise.resolve()
        ...
        .then((res) => {
          ...
          const cond = res.default != null && res.__esModule
          this.configFile = cond ? res.default : res
          return true
        })
    }
    return true
  ...
  normalizeEnvValue(value: string) {
  ...
  overwriteFromEnv(objParam: any, prefix?: string) {
    let obj = objParam
    Object.keys(obj).forEach((key) => {
      const envName = ((prefix ? `${prefix}_` : '') + key).toUpperCase()

      if (process.env[envName]) {
        obj[key] = this.normalizeEnvValue(env(envName))
      }
      ...
      Object.keys(process.env)
        ...
        .forEach((variable) => {
          const dotted = variable.withoutPrefix
            ...
            .join('.')
          obj = utils.dotSet(obj, dotted, this.normalizeEnvValue(env(variable.key)))
        })
    }
   ...
   * @returns {Service}
   */
  loadNpmModule(name: string) {
    const svc: ServiceSchema = require(name)
  ...
  startBroker() {
    ...
    if (this.worker) {
      const nodeID = `${this.config.nodeID || utils.getNodeID()}-${this.worker.id}`
      Object.assign(this.config, { nodeID })
    }
